<?php
    ...
    require_once "connection.php";
        ...
        if(count($errors) > 0){
            ...
            $query = "SELECT DISTINCT r.id AS idRoom, r.number, v.name AS view, r.size, r.num_people, r.num_beds, r.coverImage, p.price, rt.name, rt.id
                      from room_type rt INNER JOIN room r ON rt.id=r.id_room_type INNER JOIN view v ON v.id=r.id_view  
                      INNER JOIN price p ON r.id=p.id_room LEFT OUTER JOIN booking b ON b.id_room=r.id WHERE
                      rt.id = :roomType AND r.num_people >= :numPeople AND v.id = :view AND p.price BETWEEN :price1 AND :price2 AND
                      p.price = (SELECT MAX(price) FROM price WHERE id_room = r.id)
            ...
            $prepare->bindParam(":price1", $price1);
            $prepare->bindParam(":price2", $price2);
